...
pub struct PipelineResult<'c, T: crate::prelude::Protocol> {
    conn: &'c mut Conn,
    queries: Vec<u8>,
    protocol: PhantomData<T>,
}

impl<'c, T: crate::prelude::Protocol> PipelineResult<'c, T> {
    pub(crate) fn new(conn: &'c mut Conn, queries: Vec<u8>) -> Self {
        Self {
            conn,
            queries,
            protocol: PhantomData,
